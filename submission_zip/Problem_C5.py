...
Original file is located at
    https://colab.research.google.com/drive/1vGkErLRe9t2zhNX67ikH_0iLGa74nbFV
"""
...
def windowed_dataset(series, batch_size, n_past=24, n_future=24, shift=1):
    ...
    ds = tf.data.Dataset.from_tensor_slices(series).window(size = n_past + n_future, shift=shift, drop_remainder= True).flat_map(lambda w: w.batch(n_past + n_future)).shuffle(1000).map(lambda window: (window[:-n_past], window[-n_past:, :1]))
...
def solution_C5():
    ...
    model = tf.keras.models.Sequential([
        ...
        tf.keras.layers.LSTM(64, activation='relu', input_shape=(24,7), dtype='float32'),
