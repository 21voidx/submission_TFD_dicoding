...
Original file is located at
    https://colab.research.google.com/drive/1itaisACLl_Yo8aHmmxnOBj-g5Mb3EUsA
"""
...
class myCallback(tf.keras.callbacks.Callback):
  def on_epoch_end(self, epoch, logs = {}):
    if(logs.get('loss') < 0.15 and epoch > 10):
...
def solution_A5():
    ...
    with open('sunspots.csv') as csvfile:
      ...
      next(reader)
      for row in reader:
          sunspots.append(float(row[2]))  # Assuming the sunspot data is in the third column
    ...
    model.compile(optimizer='adam', loss='mae', metrics=['mae'])  # Compiling with Mean Absolute Error (MAE) as the loss
    # Fitting the model
    model.fit(train_set, epochs=50, batch_size=256, verbose=2, callbacks = [callback])  # Adjust epochs as needed
